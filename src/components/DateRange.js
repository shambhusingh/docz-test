...
const DateRange = props => {
  ...
  const isValidDate = current => {
    return (
      current.isSame(eventStartDate) ||
      current.isBetween(eventStartDate, eventEndDate) ||
      current.isSame(eventEndDate)
    )
  }
  ...
  const isToDateValid = current => {
    return (
      ...
      current.isBetween(startDate, eventEndDate) ||
      current.isSame(eventEndDate)
    )
  }
    ...
    <Segment vertical>
      <div className="filter-title">
        ...
        <a
          ...
          className="filter-segment-clear"
      ...
      <div className="date-filter">
        <Datetime
          ...
          onChange={date =>
            onDateChange('startDate', date.format('YYYY-MM-DD'))
          }
  ...
  document.addEventListener(
    ...
    function(e) {
      const position = document
        .querySelector(`.rdt-${type}-date > .form-control`)
        .getBoundingClientRect()
      const scrolledTop = position.top + position.height - 133
      document.querySelector(
        `.rdt-${type}-date .rdtPicker`,
      ).style.top = `${scrolledTop}px`
    },
    {once: true},
  )
}
